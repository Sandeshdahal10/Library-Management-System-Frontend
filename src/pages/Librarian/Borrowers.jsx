...
export default function Borrowers() {
  ...
  const fetchBorrower = async () => {
    try {
      const res = await axios.get(
        "https://library-management-system-boo3.onrender.com/api/borrowers",
        {
      ...
      if (res.status === 200) {
        ...
        try {
          const bres = await axios.get(
            "https://library-management-system-boo3.onrender.com/api/books",
          ...
          console.debug(
            "Could not fetch books to resolve titles",
            be?.toString()
          );
        }
      } else {
        console.error(
          "Unexpected status when fetching borrowers:",
          res.status,
          res.data
        );
      }
  ...
  const handleMarkReturned = async (borrowId) => {
    ...
    try {
      const res = await axios.put(
        `https://library-management-system-boo3.onrender.com/api/borrowers/${encodeURIComponent(
          borrowId
        )}`,
      ...
      setBorrower((s) =>
        s.map((b) =>
          b._id === borrowId || b.id === borrowId
            ? res.data?.borrow || res.data || b
            : b
        )
      );
  ...
  const handleDeleteBorrow = async (borrowId) => {
    ...
    try {
      ...
      setBorrower((s) =>
        s.filter((b) => !(b._id === borrowId || b.id === borrowId))
      );
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-100">
                  {(borrower || []).map((r, idx) => {
                    ...
                    const foundBorrowId =
                      r._id || r.id || r.borrowId || r.borrow_id || null;
                    // derive user display name
                    const userObj = r.user ?? r.userId ?? r.member ?? null;
                    const userName =
                      (userObj &&
                        (userObj.name || userObj.fullName || userObj.email)) ||
                      r.userName ||
                      r.user_name ||
                      String(
                        r.userId ?? r.user ?? r.memberId ?? r.member ?? "-"
                      );
                    // derive book title
                    const bookObj = r.book ?? r.bookId ?? null;
                    const bookId =
                      (bookObj && (bookObj._id || bookObj.id)) ||
                      r.bookId ||
                      r.book_id ||
                      null;
                    const bookTitle =
                      (bookObj && (bookObj.title || bookObj.name)) ||
                      (bookId && booksMap[String(bookId)]) ||
                      r.bookTitle ||
                      r.book_title ||
                      String(bookId ?? "-");
                    const borrowDate =
