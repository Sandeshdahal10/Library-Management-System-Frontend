...
export default function Borrowers() {
  ...
  const fetchBorrower = async () => {
    try {
      const res = await axios.get("https://library-management-system-boo3.onrender.com/api/borrowers", {
      ...
      if (res.status === 200) {
        ...
        try {
          const bres = await axios.get('https://library-management-system-boo3.onrender.com/api/books', { headers: token ? { Authorization: `Bearer ${token}` } : {} });
  ...
  const handleMarkReturned = async (borrowId) => {
    ...
    try {
      const res = await axios.put(`https://library-management-system-boo3.onrender.com/api/borrowers/${encodeURIComponent(borrowId)}`, { returnDate: new Date().toISOString() }, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      // update local state
  ...
  const handleDeleteBorrow = async (borrowId) => {
    ...
    try {
      const res = await axios.delete(`https://library-management-system-boo3.onrender.com/api/borrowers/${encodeURIComponent(borrowId)}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
